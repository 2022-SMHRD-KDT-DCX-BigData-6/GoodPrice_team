...
public class UpdateTb_BoardService implements Command {

    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) {
    	System.out.println("[UpdateTb_BoardService]");
        ...
        try {
            ...
            b_idx = Integer.parseInt(multi.getParameter("b_idx"));
            ...
            tb_boardDTO dto = new tb_boardDTO(b_idx, b_title, b_content, b_file, m_id);
            cnt = new tb_boardDAO().updateBoard(dto);

        } catch (IOException e) {
            e.printStackTrace();
        }
        String moveURL = null;
        moveURL = "Board_View.jsp?b_idx="+b_idx;
        if (cnt > 0) {
